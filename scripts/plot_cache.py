...
numeric_cols = ['rows', 'cols', 'threads',
                'L1_dcache_loads', 'L1_dcache_load_misses', 'L1_cache_miss_rate',
                'branch_loads', 'branch_load_misses', 'branch_miss_rate', 'time_ms']
...
df = df.dropna(subset=['L1_cache_miss_rate', 'branch_miss_rate', 'threads', 'time_ms'])
...
for matrix in matrices:
    ...
    ax1.plot(matrix_data['threads'], matrix_data['L1_cache_miss_rate'], 
...
# Scatter plot
scatter1 = ax4.scatter(avg_perf['matrix_size'], avg_perf['L1_cache_miss_rate'],
...
if len(avg_perf) > 2:
    ...
    if len(valid_data) > 2:
        z = np.polyfit(np.log10(valid_data['matrix_size']), 
                       valid_data['L1_cache_miss_rate'], 1)
        p = np.poly1d(z)
...
for idx, row in avg_perf.iterrows():
    ax4.annotate(row['matrix_name'], 
                (row['matrix_size'], row['L1_cache_miss_rate']),
                fontsize=7, alpha=0.8, 
                xytext=(5, 5), textcoords='offset points')

ax4.set_xlabel('Matrix Size (rows × cols)', fontsize=12, fontweight='bold')
ax4.set_ylabel('L1 Cache Miss Rate (%, average)', fontsize=12, fontweight='bold')
ax4.set_title('L1 Cache Miss Rate vs Matrix Size', fontsize=14, fontweight='bold')
ax4.grid(True, alpha=0.3, linestyle='--', zorder=1)
ax4.set_xscale('log')
if len(avg_perf) > 2:
    ax4.legend(loc='best', fontsize=9)
cbar4 = plt.colorbar(scatter1, ax=ax4)
...
print(f"{'Matrix':<25s} {'L1 Miss %':>10s} {'Branch Miss %':>14s} {'Time (ms)':>12s} {'Size':>12s} {'Density':>10s}")
print("=" * 95)
for idx, row in avg_perf.sort_values('L1_cache_miss_rate').iterrows():
    print(f"{row['matrix_name']:<25s} {row['L1_cache_miss_rate']:>9.2f}% {row['branch_miss_rate']:>13.2f}% "
          f"{row['time_ms']:>11.2f} {row['matrix_size']:>12,} {row['density_clean']:>9.2f}%")
...
print(f"{'Threads':<10s} {'L1 Miss %':>10s} {'Branch Miss %':>14s} {'Time (ms)':>12s}")
print("-" * 60)
for _, row in thread_summary.iterrows():
    print(f"{int(row['threads']):<10d} {row['L1_cache_miss_rate']:>9.2f}% {row['branch_miss_rate']:>13.2f}% "
          f"{row['time_ms']:>11.2f}")
...
best_cache = avg_perf.loc[avg_perf['L1_cache_miss_rate'].idxmin()]
worst_cache = avg_perf.loc[avg_perf['L1_cache_miss_rate'].idxmax()]

print(f"Best L1 cache:  {best_cache['matrix_name']} ({best_cache['L1_cache_miss_rate']:.2f}% miss rate)")
print(f"Worst L1 cache: {worst_cache['matrix_name']} ({worst_cache['L1_cache_miss_rate']:.2f}% miss rate)")
...
if len(thread_summary) > 1:
    ...
    cache_change = thread_summary.iloc[-1]['L1_cache_miss_rate'] - thread_summary.iloc[0]['L1_cache_miss_rate']
...
print("\nMetrics collected: L1 data cache loads/misses, branch-loads/misses (using --inherit for all OpenMP threads)")
