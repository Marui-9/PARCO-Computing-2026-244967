#!/usr/bin/env python3
"""
Plot MPI Communication Efficiency across node counts (1-8 nodes).
Efficiency = (Speedup / Number of Nodes) * 100% where Speedup is relative to 1-node baseline.
...
def calculate_speedup_and_efficiency(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate speedup relative to 1-node baseline and efficiency for each configuration and matrix.
    Speedup = 1node_time / current_time
    Efficiency = (Speedup / num_nodes) * 100%
    """
    df = df.copy()
    df['speedup_relative_baseline'] = 1.0
    df['efficiency_pct_1node_baseline'] = 100.0
    ...
    for config in df['config_name'].unique():
        ...
        for matrix in config_data['matrix'].unique():
            ...
            # Calculate speedup and efficiency for this config+matrix across all node counts
            mask = (df['config_name'] == config) & (df['matrix'] == matrix)
            df.loc[mask, 'speedup_relative_baseline'] = baseline_time / df.loc[mask, 'avg_time_ms']
            df.loc[mask, 'efficiency_pct_1node_baseline'] = (df.loc[mask, 'speedup_relative_baseline'] / df.loc[mask, 'num_nodes']) * 100.0
...
def load_all_results(results_dir: str) -> pd.DataFrame:
    ...
    # Calculate speedup and efficiency based on 1-node baseline
    all_df = calculate_speedup_and_efficiency(all_df)
...
def plot_efficiency(df: pd.DataFrame, output_png: str):
    ...
    for config, color in zip(configs, colors):
        ...
        # Group by num_nodes and take mean efficiency (1-node baseline)
        grouped = config_data.groupby('num_nodes').agg({
            'efficiency_pct_1node_baseline': ['mean', 'std']
        }).reset_index()
...
def print_efficiency_summary(df: pd.DataFrame):
    ...
    for num_nodes in sorted(df['num_nodes'].unique()):
        ...
        for config in sorted(node_data['config_name'].unique()):
            ...
            mean_eff = config_data['efficiency_pct_1node_baseline'].mean()
            std_eff = config_data['efficiency_pct_1node_baseline'].std()
