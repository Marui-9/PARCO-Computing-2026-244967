...
rm *8*
rm results/*8*
rm results/test_results_Xnodes/*8*
...
{
  echo "========================================================================"
  echo "MPI COMMUNICATION MODES BENCHMARK LOG (8 NODES)"
  echo "========================================================================"
...
echo "Starting MPI Communication Configurations Benchmark (8 NODES)"
echo "Executable: $EXE"
echo "Timeout per run: ${TIMEOUT_SECS}s"
echo "Iterations per mode: $ITERATIONS"
echo "Node counts: $NODE_COUNTS"
...
if [ -n "$PBS_NODEFILE" ] && [ -f "$PBS_NODEFILE" ]; then
    ...
    timeout 30 mpirun -np 8 -hostfile "$PBS_NODEFILE" hostname >> "$OUT_LOG" 2>&1 || {
