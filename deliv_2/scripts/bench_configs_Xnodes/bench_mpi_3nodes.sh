...
rm *3*
rm results/*3*
rm results/test_results_Xnodes/*3*
...
{
  echo "========================================================================"
  echo "MPI COMMUNICATION MODES BENCHMARK LOG (3 NODES)"
  echo "========================================================================"
...
echo "Starting MPI Communication Configurations Benchmark (3 NODES)"
echo "Executable: $EXE"
echo "Timeout per run: ${TIMEOUT_SECS}s"
echo "Iterations per mode: $ITERATIONS"
echo "Node counts: $NODE_COUNTS"
...
if [ -n "$PBS_NODEFILE" ] && [ -f "$PBS_NODEFILE" ]; then
    ...
    timeout 30 mpirun -np 3 -hostfile "$PBS_NODEFILE" hostname >> "$OUT_LOG" 2>&1 || {
