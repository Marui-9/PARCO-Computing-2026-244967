#!/bin/bash
# bench_mpi_1nodes.sh
# Benchmark MPI communication modes on 1 node
# Tests inter-thread communication strategies (single-node baseline)
# Usage: ./bench_mpi_1nodes.sh
set -u

EXE="./test_config_mpi"
OUT_BASE="results/test_results_Xnodes/test_config_mpi_results"
OUT_LOG="results/bench_mpi_1nodes.log"
TIMEOUT_SECS=3600      # 60 minutes per matrix combo
ITERATIONS=30          # 30 iterations per configuration

# Node counts to test - FIXED TO 1 NODE
NODE_COUNTS="1"
...
{
  echo "========================================================================"
  echo "MPI COMMUNICATION MODES BENCHMARK LOG (1 NODE)"
  echo "========================================================================"
...
if [ -n "$PBS_NODEFILE" ] && [ -f "$PBS_NODEFILE" ]; then
    ...
    timeout 30 mpirun -np 1 -hostfile "$PBS_NODEFILE" hostname >> "$OUT_LOG" 2>&1 || {
...
for num_nodes in $NODE_COUNTS; do
    ...
    echo ""
    echo "========================================================================"
    echo "Testing with $num_nodes MPI rank (node)"
    echo "========================================================================"
    echo "=== Testing with $num_nodes MPI rank ===" >> "$OUT_LOG"
