#!/bin/bash
# Download matrices optimized for benchmarking largest practical sizes
# https://sparse.tamu.edu/
#
# Matrix selection strategy:
#   - 3 small matrices (100k-300k rows): Quick testing, baseline
#   - 4 medium matrices (1M-1.6M rows): Standard workload
#   - 3 large matrices (20M-50M rows): Extreme-scale computation
#
...
#
# Data size: ~1-2 GB total
# Estimated download time: 10-30 minutes (depends on internet speed)
#
# Usage: ./download_matrices_biggest.sh
...
echo "Estimated total download size: ~1-2 GB"
echo "Estimated download time: 10-30 minutes (depending on internet speed)"
echo "=============================================="

# Base URL for SuiteSparse Matrix Collection
BASE_URL="https://suitesparse-collection-website.herokuapp.com/MM"

# 10 matrices spanning 3 size categories: small to extreme-scale
declare -A MATRICES=(
    ...
    # Medium: 1M-1.6M rows, 3M-15M nnz
    # Standard workload - all >1M rows
    ...
    # Large: 20M-50M rows, extreme-scale
    # Challenging computation - real-world graph datasets
    ["LAW/it-2004"]="it-2004"                             # 41.6M rows, 1.15B nnz
...
main() {
    echo ""
    echo "Downloading 10 matrices (36k to 41.6M rows)..."
    echo "  (~1-2 GB total, estimated time: 10-30 minutes)"
    echo ""
    ...
    echo "Naming convention: {rows_in_k}k_{density%}.mtx"
    echo ""
    echo "Matrices range: 36k to 41.6M rows"
    echo "Sparsity: 0.0018% to 0.28% (highly sparse)"
    echo ""
    echo "=============================================="
}
...
usage() {
    ...
    echo "  - Small (3):   36k-300k rows"
    echo "  - Medium (4):  916k-1.6M rows (all >1M except web-Google)"
    echo "  - Large (3):   1.8M-41.6M rows (extreme-scale)"
    echo ""
