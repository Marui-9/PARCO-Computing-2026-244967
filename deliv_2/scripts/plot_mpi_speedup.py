#!/usr/bin/env python3
"""
Plot MPI Communication Speedup across node counts (2-6 nodes).
Speedup is calculated relative to 2-node baseline for each configuration.
...
def calculate_speedup_relative_to_baseline(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate speedup relative to 2-node baseline for each configuration and matrix.
    Speedup = baseline_time / current_time (per matrix, not mixed)
    """
    df = df.copy()
    df['speedup_relative_baseline'] = 1.0
    
    # For each configuration and matrix, set baseline (2-node) time and calculate speedup
    for config in df['config_name'].unique():
        ...
        for matrix in config_data['matrix'].unique():
            ...
            baseline_data = config_matrix_data[config_matrix_data['num_nodes'] == 2]
...
def plot_speedup(df: pd.DataFrame, output_png: str):
    ...
    plt.ylabel('Speedup (relative to 2-node baseline)', fontsize=13, fontweight='bold')
...
def print_speedup_summary(df: pd.DataFrame):
    ...
    """
    print("\n" + "="*90)
    print("=== Speedup Summary (relative to 2-node baseline) ===")
    print("="*90)
...
def main():
    ...
    # Calculate speedup relative to 2-node baseline
